...
function App() {
  ...
  useEffect(()=>{
    const fetchData = async()=>{
        try {
            ...
            dispatch({type: "FETCHING_ERROR", payload: {type: "initial load", message: err.message}})
        }
    }
    fetchData() 
},[dispatch])
  ...
  return (
    <div className="App">
      {isFetching && (<div className="fetchingData">
      ...
      {error.type === "initial load" && (<div className="fetchingData">
        <p>🚫Something went wrong. Reload and wait some minutes if problem persist.</p>
        </div>) 
      }
      {posts &&
        (
        <>
          ...
          <footer>
                <p>created by <a href="https://sprintwithcarlos.github.io/" target="_blank">Sprint With Carlos</a>
         </p>
         </footer>
        </>
      )} 
